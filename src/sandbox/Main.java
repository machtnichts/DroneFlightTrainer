...
public class Main {
		...
		new Runnable(0, 1) {
			...
			public void run() {
				...
				for (Bot bot : population) {
					...
					double angle = (Vector2.SignedAngle(bot.getDir(), new Vector2(0, 1))) / 180D;
					double xToTarget = (bot.getPos().getX() - midpoint.getX()) / (double) screen.getWidth();
					double yToTarget = (bot.getPos().getY() - midpoint.getY()) / (double) screen.getHeight();
	...
	public static Bot createBot() {
		...
		bot.addTruster(new Thruster(new Vector2(60, 0), new Vector2(0, -1), 100, 5));
		bot.addTruster(new Thruster(new Vector2(-60, 0), new Vector2(0, -1), 100, 5));
